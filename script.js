document.addEventListener('DOMContentLoaded', () => {
   ...
   /*
       async function handleForwardLookup() {
        ...
        // === GHI CHÚ: Phần dọn dẹp giao diện và hiển thị mã cũ giữ nguyên ===
        const oldCodes = `${selectedCommuneValue}, ${selectedDistrictValue}, ${selectedProvinceValue}`;
        let oldAddressHtml = `
            <div class="address-line"><p><span class="label">${t('oldAddressLabel')}</span> ${fullOldAddress}</p></div>
            <div class="address-codes"><span class="label">Old Code:</span> ${oldCodes}</div>`;
        ...
        try {
            // === GHI CHÚ CỐT LÕI 1: KIỂM TRA LỊCH SỬ TĨNH VÀ CHIA TÁCH ===
            const provinceData = allProvincesData.find(p => p.code == selectedProvinceValue);
            const districtData = provinceData ? provinceData.districts.find(d => d.code == selectedDistrictValue) : null;
            const wardData = districtData ? districtData.wards.find(w => w.code == initialOldWardCode) : null;

            // Luồng 1: Nếu xã có lịch sử tĩnh, chỉ hiển thị gợi ý và dừng lại.
            ...
            // === GHI CHÚ CỐT LÕI 2: ĐƠN GIẢN HÓA LOGIC GỌI API ===
            // Loại bỏ hoàn toàn vòng lặp while. Chỉ còn một lệnh gọi API duy nhất.
            const apiUrl = `/api/lookup-forward?code=${initialOldWardCode}${isSplit ? '&is_split=true' : ''}`;
            const response = await fetch(apiUrl);
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Server error');

            // === GHI CHÚ CỐT LÕI 3: GIỮ NGUYÊN LOGIC HIỂN THỊ KẾT QUẢ ===
            if (data.is_split_case) {
                // Trường hợp CHIA TÁCH (Logic này được giữ nguyên từ phiên bản gốc của bạn)
                const splitHtml = data.split_results.map(result => {
                    if (result.new_address) {
                        ...
                        const newCodes = `${result.new_address.new_ward_code}, ${result.new_address.new_province_code}`;
                        return `<li><b>${result.description}:</b> ${t('mergedInto')} <b>${newAddress}</b><div class="address-codes"><span class="label">New Code:</span> ${newCodes}</div></li>`;
                    }
                    return `<li><b>${result.description}:</b> ${t('noMergeInfo')}</li>`;
                }).join('');
                newAddressDisplay.innerHTML = `<p class="split-case-note">${t('splitCaseNote')}</p><ul class="split-results-list">${splitHtml}</ul>`;

                if (data.split_results.length > 0 && data.split_results[0].new_address) {
                    newWardCodeForModal = data.split_results[0].new_address.new_ward_code;
                    newProvinceCodeForModal = data.split_results[0].new_address.new_province_code;
                    if (adminCenterActions) adminCenterActions.classList.remove('hidden');
                }

            } else if (data.length > 0) {
                // Trường hợp sáp nhập có ĐÍCH ĐẾN (Logic này được giữ nguyên)
     ...
     async function handleForwardLookup() {
        ...
        try {
            ...
            if (events.length === 0) {
                ...
                // Trường hợp SÁP NHẬP ĐƠN GIẢN (chỉ có 1 sự kiện và không phải SPLIT_MERGE)
                const finalUnitData = events[0];
                const newAddressForDisplay = `${finalUnitData.new_ward_name}, ${finalUnitData.new_province_name}`;
                const newCodes = `${finalUnitData.new_ward_code}, ${finalUnitData.new_province_code}`;
                const newAddressForCopy = `${newAddressForDisplay} (Codes: ${newCodes})`;
